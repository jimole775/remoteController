...
export default function (socket, data) {
  ...
  if (namesMap.indexOf(data.uid) < 0) {
    ...
    TR.getStorage("namesMap").forEach(function (item, index) {
      const curNamesMap = Array.prototype.slice.call(TR.getStorage("namesMap"))
      curNamesMap.splice(curNamesMap.indexOf(item), 1)
      const remoteChanelMap = TR.getStorage("remoteChanelMap") || []
      const curClients = TR.getStorage("clients") // 由于前面有操作，在推送之前重新获取
